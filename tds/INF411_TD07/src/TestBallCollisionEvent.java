...
public class TestBallCollisionEvent {
	...
	static public void main(String[] args) throws FileNotFoundException {
...

		{
			...
			assert billiard.eventQueue.size() == 5 : "Mauvais nombre de collisions. Appel inadéquat à Billiard.initialize() ?"
					+ " Tentative de suppresion des doublons ?";
		...
		{ 
			Billiard billiard = new Billiard("init/p100-2K.txt");
			
			billiard.eventQueue.add(new StopEvent(billiard, 100.0));
			billiard.run();

			assert 0.6665313465324731 == billiard.balls.firstElement().x;
